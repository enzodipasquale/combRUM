...
def test_gather_agent_values_counts_one_round_and_contributed_bytes() -> None:
    ...
    # int32 ids so the value (24 B) and id (12 B) contributions differ
...
def test_send_to_root_counts_one_round_zero_bytes_and_is_transparent() -> None:
    ...
    # documented zero-byte rule for this primitive
...
def test_exchange_cuts_bytes_use_documented_header() -> None:
    ...
    probe = CountingTransport(SerialTransport())
    # row 0: 5-float64 phi (40 bytes, same width as the header); row 1:
    # 3-float64 phi (24 bytes)
    rows = [
...
def test_route_agent_values_bytes_count_received_not_sent() -> None:
    # size=3, n_agents=6: contiguous sharding puts agent g on rank g // 2,
    # so the source's 6 pairs split two per rank
    def fn(t):
    ...
    # each rank receives exactly its two owned (gid -> float(gid)) pairs
    ...
    # basis would give 96 at the source and 0 elsewhere
...
def test_node_shared_bytes_counted_on_publisher_only() -> None:
    def fn(t):
        ...
        # publisher's bytes may be counted
        payload = (
            {"d": np.zeros(4)}
...
def test_sum_reproducible_bitwise_identical_wrapped_vs_unwrapped() -> None:
    ...
    values = spread_values(rng, (31, 2))
    wrapped = np.asarray(probe.sum_reproducible(values, ids))

    # the reproducibility contract: the sum is invariant under any
    # permutation of the (id, value) rows
    ...
    # transparency: the wrapper hands back the inner result untouched
...
def test_sum_vectors_reproducible_bitwise_identical_wrapped_vs_unwrapped() -> None:
    ...
    inner = SerialTransport()
    probe = CountingTransport(inner)
    values = spread_values(rng, (2, 3))
    wrapped = np.asarray(probe.sum_vectors_reproducible(values))

    # over a single rank the reproducible vector sum is the identity, so the
    # (B, M) result must be the input verbatim
...
def test_sum_vectors_reproducible_multirank_matches_hand_sum() -> None:
    # rank r contributes full((2, 3), r + 1); the sum across three ranks is
    # 1 + 2 + 3 = 6 everywhere
    n_ranks = 3
...
def test_symmetric_scenario_counts_equal_on_every_rank() -> None:
    def fn(t):
        probe = CountingTransport(t)
        # int32 ids so the value (32 B) and id (16 B) byte terms differ
    ...
    # identical on every rank because every rank feeds the same shapes:
...
def test_two_reps_one_owner_exchange_is_one_round_on_every_rank() -> None:
    ...
    # ships one row: phi (3 float64 = 24) + key b"kk" (2) + header (40) = 66
    assert [b for _, _, b in outs] == [66, 66, 66]


def test_exchange_cuts_routes_each_rep_to_its_nonzero_owner() -> None:
    # owners=[0, 1]: rep 0 belongs to rank 0, rep 1 to rank 1. rank r
    # contributes one row for rep r, so each rank gets its own rep back.
...
def test_reset_zeroes_tallies() -> None:
    probe = CountingTransport(SerialTransport())
    # single-rank cross-rank max returns the input verbatim
...
def test_snapshot_immutable_and_does_not_drift() -> None:
    probe = CountingTransport(SerialTransport())
    # seed carries 2 float64 = 16 payload bytes, matching the snapshot
    seed = np.array([3.0, 1.0])
    assert np.array_equal(np.asarray(probe.batched_max(seed)), seed)
    snap = probe.snapshot()
