...
class DualConcentration:
    ...
    Absence from the payload means "no evidence", never "settled". Both
    arrays are copied and frozen so the broadcast payload cannot be
...
class DualInformed(RepricingSchedule):
    ...
    re-priced within the last ``max_staleness`` iterations.
    ...
    def __init__(
        ...
        if (
            not isinstance(max_staleness, (int, np.integer))
            or max_staleness < 1
        ):
            raise ValueError(
                "max_staleness must be an int >= 1,"
    ...
    def select(
        ...
        return mask | ((int(iteration) - last) >= self._max_staleness)
